...
```{r}
# drop beginning trading days that have NA
NYSE_test <- NYSE |>
  filter(train == "FALSE") |>
  select(-train) |>
  drop_na()
...
wrong_split <- initial_split()



correct_split <- initial_time_split()

training(correct_split) |> mutate(type = "training"), 
(testing(correct_split) |> mutate(type = "testing")) |> 
  ggplot(aes(x = date, y = log_volume, color = type)) + geom_line()
```

```{r}
#GO BACK
rolling <- rolling_origin(
  NYSE_other |> arrange(date), initial = 30, asess = 7) |>
  #30 days, 70 days
...
en_wf <-
  ...
  add_model(en_mod) |>
  step_rm(date) #can you not remove date in the recipe??
```

```{r}
folds <- NYSE_other |> arrange(data) |>
  sliding_period(date, period = "month", lookback = Inf, #go back) |>
                 
month_folds <- NYSE_other |>
  sliding_period(date, period = "month", lookback = Inf, skip = 4)
```

```{r}
lambda_grid <- grid_regular(penalty(range = c(-8, 7), trans = log10_trans(), levels= 100))
lambda_grid
```

```{r}
en_fit <- tune_grid(en_wf, resamples = month_folds, grid = lambda_grid) |> collect_metrics()
en_fit
```
